...
export const useUserStore = create((set) => ({
  ...
  fetchUsers: () => {
    ...
    axios.get('https://localhost:7246/users', {
        headers: {
  ...
  fetchUserById: (id) => {
    ...
    axios.get(`https://localhost:7246/users/${id}`, {
        headers: {
          Authorization: GetToken()
        }
      })
  ...
  registerUser: (data) => {
    ...
    axios.post(`https://localhost:7246/users/register`, data)
      .then(res =>{
  ...
  addUser: (data, navigate) => {
    ...
    axios.post(`https://localhost:7246/users`, data, {
        headers: {
          Authorization: GetToken()
        }
      })
  ...
  updateUser: (data, id, navigate) => {
    ...
    axios.put(`https://localhost:7246/users/${id}`, data, {
        headers: {
          Authorization: GetToken()
        }
      })
  ...
  deleteUser: (id, fetchUsers) => {
    ...
    axios.delete(`https://localhost:7246/users/${id}`, {
        headers: {
          Authorization: GetToken()
        }
      })
  ...
  recoverPassword: async (data) => {
    try{
      ...
      let res = await axios.post(`https://localhost:7246/users/recover`, data);
      let emailData = {
        email: data.email,
  ...
  resetPassword: (data) => {
    axios.post(`https://localhost:7246/users/reset`, data)
    .then(res => {
      let emailData = {
        email: data.email,
        message: `Your new password is: ${res.data.password}`,
  ...
  changePassword: (data, navigate) => {
    ...
    axios.post(`https://localhost:7246/users/change`, data, {
        headers: {
          Authorization: GetToken()
        }
      })
  ...
  login: (data) => {
    ...
    axios.post('https://localhost:7246/security', data)
      .then(res =>{
